...
template<IntervalType Interval>
void step(BoxQueue& box_queue, const Polyhedron<Interval>& hole, const Polyhedron<Interval>& plug, const int iterations2, const int projection_resolution, const int rotation_resolution) {
    const std::optional<Box> optional_box = box_queue.pop();
    ...
    for(int thread = 0; thread < num_threads; thread++) {
        threads.emplace_back([&] {
            for(int iteration3 = 0; iterations3_per_thread == 0 || iteration3 < iterations3_per_thread; iteration3++) {
                step(box_queue, hole, plug, iterations2, projection_resolution, rotation_resolution);
