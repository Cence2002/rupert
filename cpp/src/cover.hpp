...
public:
    explicit CompressedId() : bits_(0) {}

    explicit CompressedId(const uint16_t bits) : bits_(bits) {}

    CompressedId(const CompressedId& other) = default;

    CompressedId& operator=(const CompressedId& other) = default;

    CompressedId(CompressedId&& other) = default;

    CompressedId& operator=(CompressedId&& other) = default;
    ...
    uint8_t depth() const {
        if(bits_ == 0) {
            throw std::runtime_error("Depth of invalid Id is undefined");
        }
        return static_cast<uint8_t>(16 - __builtin_clz(bits_));
    }
    ...
    double len() const {
        return std::pow(0.5, depth());
    }

    CompressedId min() const {
        if(bits_ == 0) {
    ...
    std::pair<CompressedId, CompressedId> split() const {
        if(bits_ == 0) {
            throw std::runtime_error("Invalid Id cannot be split");
        }
        return std::make_pair(min(), max());
    }
    ...
    Interval interval() const {
        if(bits_ == 0) {
            throw std::runtime_error("Invalid Id has no interval");
        }
        Interval interval = Interval::unit();
        for(uint8_t shift = depth(); shift > 0; shift--) {
            if(bits_ & (1 << (shift - 1))) {
                interval = Interval(interval.mid(), interval.max());
            } else {
                interval = Interval(interval.min(), interval.mid());
    ...
    friend std::ostream& operator<<(std::ostream& os, const CompressedId& id) {
        return os << "<" << std::bitset<16>(id.bits_).to_string().substr(16 - id.depth()) << ">";
    }
};
