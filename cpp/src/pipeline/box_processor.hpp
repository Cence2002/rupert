...
private:
    ...
    std::optional<TerminalBox> get_optional_terminal_box(const Box& box) {
        Polygon<Interval> projected_hole = project_hole(box, config_.hole(), config_.projection_resolution(), config_.rotation_resolution());
...
public:
    ...
    bool process() {
        ...
        // debug_exporter.cover_builder.box_builder.set_complete(optional_terminal_box.has_value());
