...
public:
    ...
    Plot(int width, int height, float range) {
        ...
        origin = Eigen::Vector2f(width / 2, height / 2);
    ...
    void box(const Box &box, const cv::Scalar &color = Color::WHITE, int thickness = 1) {
        Eigen::Vector2f bottom_left = {box.theta_interval.min, box.phi_interval.min};
        Eigen::Vector2f top_left = {box.theta_interval.min, box.phi_interval.max};
        Eigen::Vector2f top_right = {box.theta_interval.max, box.phi_interval.max};
        Eigen::Vector2f bottom_right = {box.theta_interval.max, box.phi_interval.min};
